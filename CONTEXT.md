...
| **proxy-first / 客户端兜底** | 配 `VITE_TICKET_PROXY_BASE_URL` 时**全平台**走同仓 Express 代理(配了就不会走客户端,见 ADR-0005「生效条件」);未配 / 不可达时走客户端:Lawson 用 Cronet(Chromium 栈),其余用 CapacitorHttp。见 ADR-0002 / ADR-0005。 |
...
| Lawson(ローチケ) | ✅ Android 端上 Cronet 直取(ADR-0005,~400ms);iOS 无插件需代理 | ✅ 搜索页 ResultBox 每公演×每轮全窗口(2026-07);失败时给官方跳转 |
