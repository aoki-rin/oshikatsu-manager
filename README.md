...
未配代理 → 端上直取：Lawson 走 Cronet，其余走 CapacitorHttp
...
- **代理可选，端上自足**：配置 `VITE_TICKET_PROXY_BASE_URL` 时**全平台**走代理（配了就不会走端上）；留空则直接端上直取。Android 上 Lawson 走 Cronet（Chromium 网络栈），因此**不再需要常驻 Mac + Tailscale**——见 [ADR-0005](docs/adr/0005-lawson-cronet-on-device.md)。iOS 无该插件，要搜 Lawson 仍需代理。
...
| **Lawson** | `l-tike.com/search/?keyword=` | ✅ Android 端上 Cronet 直取（~400ms，ADR-0005）；iOS 需代理。失败时明确报因并提供ローチケ跳转 |
