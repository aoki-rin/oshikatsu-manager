...
// Real scraped data (generated by scraper/build-events.mjs from official pages)
import realData from './data/events.json';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 2. Events & Plugins — real scraped data (events.json) always merged on top of
    //    stored/mock data, deduped by id, so it shows regardless of any stale cache.
    const realEvents = realData.events as unknown as ActivityEvent[];
    const realArtists = realData.artists as unknown as Artist[];
    const realVenues = realData.venues as unknown as Venue[];

    const storedEvents = localStorage.getItem('oshikatsu_events');
    const baseEvents: ActivityEvent[] = storedEvents ? JSON.parse(storedEvents) : INITIAL_EVENTS;
    const realEventIds = new Set(realEvents.map(e => e.id));
    setEvents([...realEvents, ...baseEvents.filter(e => !realEventIds.has(e.id))]);

    const storedArtists = localStorage.getItem('oshikatsu_artists');
    const baseArtists: Artist[] = storedArtists ? JSON.parse(storedArtists) : INITIAL_ARTISTS;
    const realArtistIds = new Set(realArtists.map(a => a.id));
    setArtists([...realArtists, ...baseArtists.filter(a => !realArtistIds.has(a.id))]);

    const storedVenues = localStorage.getItem('oshikatsu_venues');
    const baseVenues: Venue[] = storedVenues ? JSON.parse(storedVenues) : INITIAL_VENUES;
    const realVenueIds = new Set(realVenues.map(v => v.id));
    setVenues([...realVenues, ...baseVenues.filter(v => !realVenueIds.has(v.id))]);
    ...
    if (storedFollowedArt) {
      ...
      setFollowedArtists(['art-ikimonogakari', 'art-yoasobi', 'art-chika-stars', 'art-hatsune']);
    }
